...
GO_BUILD_TARGET ?= ./cmd/ioctl
VERSION ?=
COMMIT ?=
BUILD_TIME ?=
GO_LDFLAGS ?= -s -w
WINDOWS_BINARY = $(if $(filter %.exe,$(BINARY)),$(BINARY),$(BINARY).exe)

ifneq ($(strip $(VERSION)),)
GO_LDFLAGS += -X github.com/photowey/iocgo/cmd/ioctl/version.Version=$(VERSION)
endif

ifneq ($(strip $(COMMIT)),)
GO_LDFLAGS += -X github.com/photowey/iocgo/cmd/ioctl/version.Commit=$(COMMIT)
endif

ifneq ($(strip $(BUILD_TIME)),)
GO_LDFLAGS += -X github.com/photowey/iocgo/cmd/ioctl/version.BuildTime=$(BUILD_TIME)
endif

.DEFAULT_GOAL := help

.PHONY: help clean deps download tidy verify-mod fmt fmt-check vet test test-race lint check build build-linux build-linux-arm64 build-windows build-macos build-macos-arm64 linux windows macos

GO_RUN = env GOCACHE="$(GOCACHE)" GOMODCACHE="$(GOMODCACHE)" GOPROXY="$(GOPROXY)" GOFLAGS="$(GOFLAGS)" $(GO)
GO_BUILD = env GOCACHE="$(GOCACHE)" GOMODCACHE="$(GOMODCACHE)" GOPROXY="$(GOPROXY)" GOFLAGS="$(GOFLAGS)" GOOS="$(GOOS)" GOARCH="$(GOARCH)" CGO_ENABLED="$(CGO_ENABLED)" $(GO)

## Show this help.
help:
	@{ \
		if [ -t 1 ] && command -v tput >/dev/null 2>&1 && [ "$$(tput colors 2>/dev/null || echo 0)" -ge 8 ]; then \
			C_CYAN="$$(tput setaf 6)"; C_RESET="$$(tput sgr0)"; \
		else \
			C_CYAN=""; C_RESET=""; \
		fi; \
		echo; \
		...
		echo "Examples:"; \
		echo "  make build"; \
		echo "  make build VERSION=v0.1.0"; \
...
build: clean ## go build -o ioctl ./cmd/ioctl
	$(GO_BUILD) build -ldflags "$(GO_LDFLAGS)" -o "$(BINARY)" "$(GO_BUILD_TARGET)"
...
build-linux-arm64: ## GOOS=linux GOARCH=arm64
	$(MAKE) build GOOS=linux GOARCH=arm64

## Build Windows amd64 binary.
build-windows: ## GOOS=windows GOARCH=amd64
	$(MAKE) build GOOS=windows GOARCH=amd64 BINARY="$(WINDOWS_BINARY)"
